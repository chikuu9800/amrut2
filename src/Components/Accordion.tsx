...
export default function Accordion() {
  ...
  const handleSchemeClick = (title: string) => {
    navigate(`/scheme/${encodeURIComponent(title)}`);
  };
        ...
        >
          ...
          <motion.div variants={containerVariants} className="font-martel">
            {accordionData.map((item, index) => (
              ...
              >
                <motion.button
                  onClick={() => typeof item === 'string' 
                    ? handleSchemeClick(item)
                      ...
                      {item.submenu.map((sub, subIndex) => (
                        <div key={subIndex} className="mb-1">
                          {typeof sub === "object" ? (
                            <>
                              <motion.button
                                onClick={() => handleSchemeClick(sub.title)}
                                whileHover={{ x: 2 }}
                            ...
                            <motion.div
                              className="pl-2 py-1 text-gray-700 font-martel cursor-pointer hover:text-blue-600"
                              whileHover={{ x: 5 }}
                              onClick={() => handleSchemeClick(sub)}
                            >
                              {sub}
                            </motion.div>
                          )}
